...
- Реальная платёжная форма, списание денег, создание заказа и возвраты принадлежат внешним системам платформы
- Frontend не вызывает внутренний `/internal/v1/payment-events` и не создаёт `purchased` или `payment_failed` самостоятельно
