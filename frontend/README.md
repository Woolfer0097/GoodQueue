...
> Пользовательский интерфейс очереди на покупку дефицитных товаров для классифайд-платформы.
...
интерфейс предлагает релевантные и доступные похожие лоты**.
...
Базовый сценарий очереди решает проблему конкурентной покупки. Для классифайд-платформы
...
### Почему рекомендации релевантны

Высокая релевантность достигается на backend. Он возвращает товары, которые
доступны для покупки прямо сейчас, и ранжирует их в одном из двух режимов:

| Режим              | Как выбираются предложения                                                                  |
| ------------------ | ------------------------------------------------------------------------------------------- |
| `ai_semantic`      | OpenAI embeddings и pgvector учитывают смысл названия и описания, категорию, цену и остаток |
| `catalog_fallback` | Детерминированный алгоритм учитывает категорию, близость цены и свободный остаток           |

Исходный и недоступные товары исключаются. Если AI выключен, не отвечает или превышает
timeout, рекомендации продолжают работать через сохранённые векторы или fallback. ИИ
влияет только на порядок альтернатив и не участвует в очереди, резервировании или
подтверждении покупки.

Frontend не выбирает режим рекомендаций и не пересчитывает их релевантность. Он проверяет
ответ API, сохраняет порядок backend и показывает готовые релевантные карточки.

| Дополнительная задача                   | Что реализовано                                                        | Продуктовый эффект                                                        |
| --------------------------------------- | ---------------------------------------------------------------------- | ------------------------------------------------------------------------- |
| Сохранить интерес во время ожидания     | Релевантные товары доступны прямо на экране очереди                    | Пользователь продолжает изучать каталог, пока место в очереди сохраняется |
...
- Checkout-экран с подтверждённым правом, таймером и явно обозначенной безопасной demo-оплатой
...
| ИИ используется только для рекомендаций        | Очередь и резерв не зависят от внешней модели                            |
...
| `GET`    | `/api/v1/products/:productID/alternatives`                           | Релевантные доступные товары                  |
...
| `POST`   | `/api/v1/products/:productID/queue-attempts/:attemptID/demo-payment` | Идемпотентно завершить безопасную demo-оплату |

Актуальный контракт можно проверить в Swagger UI backend по пути `/docs`.
Пустой `image_url` означает, что у товара нет изображения: frontend показывает нейтральную
заглушку и продолжает отображать каталог.
...
- `X-User-ID` — демонстрационная замена идентичности после внешней авторизации, а не production-механизм аутентификации
- Реальная платёжная форма, списание денег, создание заказа и возвраты принадлежат внешним системам платформы; кнопка MVP прозрачно имитирует только успешный результат
- Frontend вызывает безопасный пользовательский demo-endpoint, но не обращается к внутреннему `/internal/v1/payment-events` и не меняет состояние покупки самостоятельно
- Countdown нужен только для отображения. Истечение срока подтверждается повторным запросом к backend
- Обновление активной попытки реализовано polling-запросами. WebSocket/SSE и push-уведомления не входят в MVP
- Demo user selector позволяет переключаться между независимыми попытками. В реальном продукте идентичность должна приходить из доверенного auth-контекста
