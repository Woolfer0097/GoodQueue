...
Глобальный монитор активных попыток опрашивает `/queue-entries/active` каждые 5 секунд на
любой странице SPA. Первый ответ в рамках текущей вкладки считается исходным снимком. При
последующем переходе состояния frontend открывает актуальный этап даже из каталога или со
страницы другого товара. Если за один polling изменилось несколько попыток, приоритет у
`invited`, затем у `checkout`, затем у terminal-состояний. Если попытка исчезает из active
списка, frontend запрашивает её итоговое состояние и открывает result-экран.

Popup показывается только когда изменилось место попытки в состоянии `waiting`. Он содержит
ссылку на очередь, закрывается только пользователем и не восстанавливается после перезагрузки
страницы.
