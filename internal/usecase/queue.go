...
func (useCase *QueueUseCase) Join(ctx context.Context, productID domain.ProductID, userID domain.ExternalUserID, idempotencyKey uuid.UUID) (domain.QueueEntry, error) {
	if _, err := useCase.products.GetByID(ctx, productID); err != nil {
...
func (useCase *QueueUseCase) Leave(ctx context.Context, productID domain.ProductID, userID domain.ExternalUserID) (domain.QueueEntry, error) {
	if err := useCase.queue.Leave(ctx, productID, userID); err != nil {
		return domain.QueueEntry{}, err
	}
	return useCase.queue.Current(ctx, productID, userID)
}
