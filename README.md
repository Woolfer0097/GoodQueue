...
### Эксперимент с размером буфера

Чтобы менять процент на основании данных, а не предположений, PostgreSQL-режим предоставляет `GET /internal/v1/queue-buffer-metrics?window=168h`. Отчёт содержит текущий `waiting_buffer_percent`, число входов, выданных и ещё активных прав, покупки, expiry, отмены, ошибки оплаты, а также среднее и p95 времени ожидания по товарам. Метрики вычисляются из сохранённой истории попыток и не сбрасываются после перезапуска backend.

`resolved_right_conversion_percent` считается как `purchases / resolved_rights × 100`. Активные `invited` и `checkout` не входят в знаменатель: их исход ещё неизвестен, поэтому они не должны искусственно снижать конверсию. Окно задаётся Go duration от `1m` до `2160h` (90 дней), по умолчанию — `168h`.

Рекомендуемый эксперимент — запустить одинаковые группы товаров с разными значениями `GOODQUEUE_WAITING_BUFFER_PERCENT`, дождаться достаточного числа завершённых прав и сравнить конверсию вместе со временем ожидания. Процент намеренно не меняется автоматически: ручное изменение конфигурации или A/B-платформа сохраняют контроль, позволяют учесть размер выборки и не создают нестабильную петлю обратной связи.
...
| Метод | Путь | Назначение |
|---|---|---|
| `GET` | `/internal/v1/queue-buffer-metrics?window=168h` | метрики эксперимента размера очереди и конверсии выданных прав |
